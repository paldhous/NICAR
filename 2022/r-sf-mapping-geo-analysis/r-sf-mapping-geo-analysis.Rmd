...
To use **rnaturalearth**, you will also need to install the **rgdal** package.
...
-   **Equirectangular**: `4326` Plots degrees of latitude against degrees of longitude. Used in the code above.

-   **Web Mercator**: `3857` Used for OpenStreetMap and other web maps. Designed for navigation, keeps direction/compass bearing constant.

-   **Albers Equal Area Conic**, centered on continental United States: `5070` Keeps areas constant, widely used for maps of the entire US.

-   **Mollweide**: `54009` Equal area projection suitable for mapping the entire world.

-   **Robinson**: `54030` A "compromise" projection for showing the entire world, minimizes distortion across each of area, shape, distance, and compass bearing but represents none of them perfectly.

[Search here](https://epsg.io/) for EPSG codes.
...
### Make a map showing the annual risk of damaging quakes in the continental United States

Before making a map from the `seismic` data, we will convert the `ValueRange` variable from text to a categorical variable, or **factor**, with the values arranged in increasing order of risk. This will ensure that the values display in correct order on the map legend.
...
`coord_sf` applies a custom projection to the map. `crs = 5070` applies an Albers Equal Area Conic projection, centered on the continental United States; `default_crs = 4326` defines the pre-existing projection of the data, so that the conversion is handled correctly.
...
Now we will add data on earthquakes to the map. The code below loads GeoJSON returned from the US Geological Survey [earthquakes search](https://earthquake.usgs.gov/earthquakes/search/) API for quakes larger than magnitude 5.5 since the beginning of 1960 within 3,000 kilometers of the geographic center of the continental United States:
...
Again, the projection is `EPSG:4326`. Now we can add the quakes to the map:
...
### Add a basemap showing neighboring countries, and zoom into the continental United States
...
[Natural Earth](https://www.naturalearthdata.com/) is a widely used repository of geodata, and can be accessed from R using the [**rnaturalearth**](https://cran.r-project.org/web/packages/rnaturalearth/vignettes/rnaturalearth.html) package. This code will load a world map with national borders.
...
In the `ne_download` function, `returnclass = sf` ensures the data is imported as an **sf** object; `scale` defines the level of detail for features on the map. For general guidance, use `10` for local or regional maps, `50` for maps of countries, and `110` for maps of the entire world; `type` is derived from the name of the file if downloaded directly from Natural Earth.
...
Adding longitude coordinates with `xlim` and latitude coordinates with `ylim` to the `coord_sf` function zoomed the map further in.

### Further geographic data processing and analysis

The **sf** package includes many functions for working with geodata that allow you to calculate distances, areas, and so on, or to perform geometric operations such finding the intersections between two geometries, drawing a bounding box around features, and so on. [Here](https://r-spatial.github.io/sf/reference/index.html) is a full reference guide.
...
If we want to work with distances, we should first convert to a projection that uses meters as its units, rather than the degrees of latitude and longitude used by `EPSG:3426`.
...
# check units of that projection
st_crs(sf_addresses)
```
...
To do this, we will first convert the projection of the `seismic` data to Web Mercator, or `EPSG:3857`, so that the area calculations are made in square meters. Then we will combine the multiple polygons for each risk category into one polygon per category. This can be done using `summarize` after first grouping by `ValueRange` with the **dplyr** function `group_by`.

```{r}
# change projection to Web Mercator so units are in meters
seismic <- seismic %>%
  st_transform(3857)
...
**sf** also works very well with the [**tidycensus**](https://walker-data.com/tidycensus/) package, which makes it easy to import data from key US Census Bureau APIs. See the vignette on [Spatial data in ](https://walker-data.com/tidycensus/articles/spatial-data.html) [**tidycensus**](https://walker-data.com/tidycensus/articles/spatial-data.html) for more details. To use **tidycensus**, you will need to obtain a [Census API key](https://api.census.gov/data/key_signup.html).
<br>
<br>
